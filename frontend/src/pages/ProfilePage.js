            ...
            <div style={s.profileCard}>
                <div style={s.avatarLarge}>
                    {profileUser.profilePic ? (
                        <img src={`https://sphere-backend-2mx3.onrender.com${profileUser.profilePic}`} alt="Profile" style={{width: '100%', height: '100%', borderRadius: '50%', objectFit: 'cover'}} />
                    ) : (
                ...
                {editMode ? (
                    <form onSubmit={handleUpdateProfile} style={s.editForm}>
                        ...
                        <textarea value={bio} onChange={(e) => setBio(e.target.value)} placeholder="Write a bio..." style={s.bioInput} />
                        ...
                        {post.imageUrl ? (
                            isVideo(post.imageUrl) ? (
                                <div style={{position: 'relative', width: '100%', height: '100%'}}>
                                    <video src={`https://sphere-backend-2mx3.onrender.com${post.imageUrl}`} style={s.gridImage} muted />
                                    <div style={s.videoIcon}><FaVideo color="white" /></div>
                                </div>
                            ) : <img src={`https://sphere-backend-2mx3.onrender.com${post.imageUrl}`} alt="Post" style={s.gridImage} />
                        ) : <div style={s.textPostPreview}><p>{post.content.substring(0, 30)}...</p></div>}
            ...
            {selectedPost && (
                <div style={s.modalOverlay} onClick={() => setSelectedPost(null)}>
                    <div style={s.modalContent} onClick={(e) => e.stopPropagation()}>
                        <div style={s.modalHeader}><span>Post Details</span><button onClick={() => setSelectedPost(null)} style={s.closeBtn}><FaTimes size={20} /></button></div>
